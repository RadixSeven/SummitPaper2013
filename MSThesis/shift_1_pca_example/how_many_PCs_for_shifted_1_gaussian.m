function [num_pcs_without_rounding, num_pcs_with_rounding, error_with_rounding_including_up_to_pc, error_without_rounding_including_up_to_pc] = how_many_PCs_for_shifted_1_gaussian( )
% Runs experiment looking at noisless reconstruction for shifted 1 to determine how many PC's SHOULD be extracted
%
% Identical to the how_many_PCs_for_shifted_1 function except that it uses
% gaussian noise instead of bit-flipping.
%
%
...
coeff_data = shifted_1_dataset(50000,20,66598278,'gaussian');
coeff=princomp(coeff_data);
col_means = mean(coeff_data);


fprintf('Generating selection dataset\n');
[sel_data,sel_noiseless] = shifted_1_dataset(5000,20,46561034,'gaussian');
...
[test_data,test_noiseless] = shifted_1_dataset(5000,20, 8985614, 'gaussian');
