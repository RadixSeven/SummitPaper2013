...
I do not use it in this paper. Skip-gram, however, I use extensively. Skip-gram 
...
output vector. The output vector is used when a word is the center, target 
word whose context is being predicted and the input vector is used when a word 
is a candidate. After training, the input vectors will have the distributed 
semantic representation seek. During training, the dot-product of these vectors 
...
variance then the technique starts with Principal Components Analysis (PCA).
...
approximation\footnote{in a least-squares sense} that can be made with n
